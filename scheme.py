...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if type(procedure) == MacroProcedure:
...
def eval_all(expressions, env):
    ...
    while expressions != nil:
        ...
        curr_eval = scheme_eval(first, env, True)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        #env.define(target, do_lambda_form(Pair(expressions.first.second, expressions.second), env))
...
def do_and_form(expressions, env):
    ...
    if expressions.second == nil:
        ...
        if scheme_truep(curr_val) == False:
            return curr_val
...
def make_let_frame(bindings, env):
    ...
    def symbol_helper(bindings):
        if bindings == nil:
            return nil
        else:
            check_form(bindings.first, 2, 2)
            return Pair(bindings.first.first, symbol_helper(bindings.second))

    symbols = symbol_helper(bindings)
    check_formals(symbols)

    def def_helper(bindings):
        if bindings == nil:
            return nil
        else:
            check_form(bindings.first, 2, 2)
            return Pair(scheme_eval(bindings.first.second.first, env), def_helper(bindings.second))

    defs = def_helper(bindings)


    return env.make_child_frame(symbols, defs)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        target = expressions.first.first
        macro_procedure = MacroProcedure(expressions.first.second, expressions.second, env)
        env.define(target, macro_procedure)
        return target
